...
sigmaList = logspace(-8, 0, 20);

rseList = zeros(algLength, length(sigmaList));
...
for cn = caseNum
    ...
    for sg_k = 1:length(sigmaList)
        sigma = sigmaList(sg_k);
    ...
    if flag_plot
        ...
        loglog(sigmaList, rseList', '-o', "LineWidth", 2)
        legend(alg_name, 'Location', 'southeast')
        xlim([min(sigmaList), max(sigmaList)]);
        xticks(10.^(-8:1:1));
        xticklabels({'10^{-8}', '10^{-7}', '10^{-6}', '10^{-5}', '10^{-4}', ...
                '10^{-3}', '10^{-2}', '10^{-1}', '10^{0}', '10^{1}'});
        xlabel('Noise Level $\varepsilon$', 'Fontsize', 16, 'Interpreter', 'latex')
        ylabel('res($\tilde{C}$)', 'Fontsize', 16, 'Interpreter', 'latex');
        if flag_plot_save
            set(gcf, 'WindowState', 'maximized');
            exportgraphics(gca, 'output/noise_vs_res_all10.png')
        end
    end
end